...
class Problem(Module.Problem):
    ...
    def fun_u(self, x:torch.tensor)->torch.tensor:
        ...
        sqrt3_torch = torch.tensor(math.sqrt(3), dtype = self._torch_dtype)
        
        cond1 = (x>= -2) * (x< -2+sqrt3_torch)
        cond2 = (x>= -2+sqrt3_torch) * (x< 2-sqrt3_torch)
        cond3 = (x>= 2-sqrt3_torch) * (x<=2)
        
        x1 = x[cond1]
        x2 = x[cond2]
        x3 = x[cond3]
        
        u[cond1] = (4-2*sqrt3_torch)*(x1+2)
        u[cond2] = 1-x2*x2
        u[cond3] = (4-2*sqrt3_torch)*(2-x3)
    ...
    def fun_f(self, x:torch.tensor)->torch.tensor:
        ...
        f[cond1] = 4-2*sqrt3_torch
        f[cond2] = -(2*sqrt3_torch-2)
        f[cond3] = -(4-2*sqrt3_torch)
