...
class GenData():
    ...
    def __init__(self, d: int, x_lb: np.array, x_ub: np.array, dtype: np.dtype=np.float64):
        '''
    ...
    def get_bd(self, N_bd_each_face:int)->list[torch.tensor]:
        ...
        for d in range(self.d):
            ...
            x_list.extend([torch.from_numpy(x_lb.astype(self.dtype)), torch.from_numpy(x_ub.astype(self.dtype))])
