...
class Solver(Module.Solver):
    ...
    def __init__(self, problem: Problem, **kwargs):
        ...
        Input:
            ...
            model_type: 'FeedForward' or 'FeedForward_Constraint'
    ...
    def train(self, save_path:str, load_type:str=None)->None:
        ...
        self.get_net()
        try:
            self._load(self.model, load_path=save_path, load_type=load_type)
            print('  A trained model has been loaded ......\n')
        except:
            print('  Started with a new model ......\n ')
            
        self.problem.eta = self.eta
        #######################################################################
        ...
        for iter in range(self.Iter_adam):
            ...
            data_train = self.problem.get_point_cube(GenData = self.problem._gen_data, 
                                                     N_xin = self.N_int, N_xbd_each_face = self.N_bd_each_face)
        ...
        for epoch in range(self.Iter_lbfgs):
            ...
            if (epoch + 1) % 10 == 0:
                loss_train, loss_all = self.get_loss(data_train)
                loss = loss_all[0]
                self._print(epoch+1, loss_train.item(), loss[0].item(), loss[1].item(), l2_err.item(), linf_err.item())
    ...
    def predict(self, load_path:str, load_type:str)->None:
        ...
        u_test, x_test = self.problem.get_test()
        with torch.no_grad():
            u_pred = self.model(x_test.to(self.device))
        # save result
        dict_test = {}
    ...
    def plot_fig(self, load_path:str)->None:
        ...
        plt.xlabel("epoch")
        plt.legend()
        plt.savefig(fig_path+'loss.png', dpi = 500, bbox_inches = 'tight')
        plt.show()
        
        plt.figure(2)
        plt.plot(epochs, np.log10(l2_err_list), 'b--', label = r"$\log_{10}(\|u_{\rm exact}-u_{\rm DNN}\|_{L^{2}}/\|u_{\rm exact}\|_{L^{2}})$")
        plt.plot(epochs, np.log10(linf_err_list), 'r--', label = r"$\log_{10}(\|u_{\rm exact}-u_{\rm DNN}\|_{L^{\infty}}/\|u_{\rm exact}\|_{L^{\infty}})$")
        plt.xlabel("epoch")
        plt.legend()
        plt.savefig(fig_path+'error.png', dpi = 500, bbox_inches = 'tight')
        plt.show()
        ...
        x = torch.from_numpy(x_test.reshape(-1,1).astype(self.numpy_dtype))
        psi = self.problem.fun_psi(x.to(self.device))
        psi_np = psi.detach().cpu().numpy()
        
        
        plt.figure(3)
        plt.plot(x_test, u_test, 'r', label = r"$u_{\rm exact}$")
        plt.plot(x_test, u_pred, 'b--', label = r"$u_{\rm DNN}$")
        plt.plot(x_test, psi_np, 'k', label = r"$\psi$")
        plt.legend()
        plt.savefig(fig_path+'solution.png', dpi = 500, bbox_inches = 'tight')
        plt.show()
        

        plt.figure(4)
        plt.plot(dict_test['x_test'], np.abs(u_test-u_pred), 'r--', label = r"$|u_{\rm exact}-u_{\rm DNN}|$")
        plt.legend()
        plt.savefig(fig_path+'difference.png', dpi = 500, bbox_inches = 'tight')
        plt.show()
        
        print(f'\n  Save the figure results from:\n  {load_path}figures')
        #######################################################################
