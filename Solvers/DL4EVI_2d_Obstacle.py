...
class Solver(Module.Solver):
    ...
    def get_loss(self, data_point:dict, **args):
        ...
        # loss_bd134 = torch.mean( eq4**2 ) 
        loss_all.append([loss_in.detach(), loss_bd.detach()])
        # loss_all.append([loss_in.detach(), loss_bd134.detach()])
        
        if self.model_type == 'FeedForward_Constraint' or 'FeedForward_Partial_Constraint':
            loss_train = loss_in
    ...
    def plot_fig(self, load_path:str)->None:
        ...
        y_mesh = x_test[:,1:].reshape(length, length)
